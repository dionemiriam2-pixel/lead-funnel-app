...
export async function POST(req) {
  ...
  const { error } = await sb.from("search_terms").insert({
    term: body.term, location: body.location || null,
    client: body.client || null, industry: body.industry || null,
    product: body.product || null, max_results: Number(body.max_results) || 20,
  });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
