...
export default function LeadForm({ lp, accent }) {
  const ac = accent || "#e8600a";
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ height: 5, background: "#e5e7eb", borderRadius: 999 }}>
          <div style={{ height: 5, width: pct + "%", background: ac, borderRadius: 999, transition: "width .35s ease" }} />
        </div>
      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {intents.map(opt => (
              <button key={opt} onClick={() => { setIntent(opt); setStep(2); }}
                style={{
                  padding: "13px 16px", border: "2px solid " + (intent === opt ? ac : "#e5e7eb"),
                  borderRadius: 10, background: intent === opt ? "#fafafa" : "#fff",
                  color: "#111827", fontSize: 15, textAlign: "left", cursor: "pointer",
                  fontWeight: intent === opt ? 700 : 400, transition: "all .15s",
                  display: "flex", alignItems: "center", gap: 10,
                }}>
                <span style={{ width: 20, height: 20, borderRadius: "50%", border: "2px solid " + (intent === opt ? ac : "#d1d5db"), background: intent === opt ? ac : "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
const nextBtn = {
  flex: 1, padding: "13px 18px", background: "#111111", color: "#fff",
